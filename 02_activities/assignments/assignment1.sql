...
SELECT vendor_id, count(vendor_id) as assignments_count FROM vendor_booth_assignments GROUP BY vendor_id;
...
SELECT customer.customer_id, customer_first_name, customer_last_name, cost FROM customer
INNER JOIN
(SELECT customer_id, SUM(quantity * cost_to_customer_per_qty) as cost FROM customer_purchases
GROUP BY customer_id HAVING cost > 2000) as c
...
SELECT customer_id, STRFTIME('%m', market_date) as month, STRFTIME('%Y', market_date) as year, SUM(quantity*cost_to_customer_per_qty) as total_money_spent
